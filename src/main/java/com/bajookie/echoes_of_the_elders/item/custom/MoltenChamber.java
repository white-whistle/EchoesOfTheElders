...
public class MoltenChamber extends Item implements IArtifact, IStackPredicate, IHasCooldown {
    ...
    private void endUse(PlayerEntity player, int remainingUseTicks, ItemStack stack, World world) {
        ...
        MagmaBullet bullet = new MagmaBullet(world,player.getEyePos().x,player.getEyePos().y,player.getEyePos().z,damage*this.shotDamage.get(stack),player);
        bullet.setVelocity(player,player.getPitch(),player.getYaw(),player.getRoll(),3f,0);
